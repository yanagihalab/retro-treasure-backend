...
(function () {
  ...
  function mat(scene, name, color, options = {}) {
    ...
    material.emissiveColor = options.glow
      ? color.scale(options.glow)
      : color.scale(0.06);
    material.specularColor =
      options.specular || new BABYLON.Color3(0.78, 0.68, 0.42);
  ...
  function createAvatar(scene, materials) {
    ...
    const coat = BABYLON.MeshBuilder.CreateCylinder(
      "avatarCoat",
      {
        diameterTop: 0.72,
        diameterBottom: 1.22,
    ...
    const scarf = BABYLON.MeshBuilder.CreateTorus(
      "avatarScarfRing",
      {
        diameter: 0.96,
        thickness: 0.045,
    ...
    const cloak = BABYLON.MeshBuilder.CreateCylinder(
      "avatarCloak",
      {
        diameterTop: 0.64,
        diameterBottom: 1.46,
  ...
  function createScene(engine, canvas) {
    ...
    const key = new BABYLON.DirectionalLight(
      "avatarKey",
      new BABYLON.Vector3(-0.45, -0.72, 0.4),
      scene,
    );
    ...
    const rim = new BABYLON.PointLight(
      "avatarRim",
      new BABYLON.Vector3(-2.6, 1.7, -2.2),
      scene,
    );
    ...
    const materials = {
      armor: mat(scene, "avatarArmor", new BABYLON.Color3(0.18, 0.28, 0.36), {
        specular: new BABYLON.Color3(0.9, 0.78, 0.44),
      }),
      cloak: mat(
        scene,
        "avatarCloakMat",
        new BABYLON.Color3(0.04, 0.11, 0.19),
        { alpha: 0.86, backFaceCulling: false },
      ),
      coat: mat(scene, "avatarCoatMat", new BABYLON.Color3(0.08, 0.18, 0.29)),
      cyan: mat(scene, "avatarCyanGlow", new BABYLON.Color3(0.27, 0.88, 1), {
        glow: 0.82,
      }),
      gold: mat(scene, "avatarGold", new BABYLON.Color3(1, 0.72, 0.27), {
        glow: 0.24,
      }),
      hair: mat(scene, "avatarHairMat", new BABYLON.Color3(0.05, 0.07, 0.1)),
      red: mat(scene, "avatarAccentRed", new BABYLON.Color3(0.9, 0.18, 0.24), {
        glow: 0.2,
      }),
    ...
    platform.material = mat(
      scene,
      "avatarPlatformMat",
      new BABYLON.Color3(0.05, 0.14, 0.23),
      { alpha: 0.72, backFaceCulling: false },
    );

    const ringA = BABYLON.MeshBuilder.CreateTorus(
      "avatarDataRingA",
      {
        diameter: 3.92,
        thickness: 0.026,
    ...
    const ringB = BABYLON.MeshBuilder.CreateTorus(
      "avatarDataRingB",
      {
        diameter: 2.28,
        thickness: 0.02,
    ...
    const scanLines = [];
    for (let i = 0; i < 6; i += 1) {
      const line = BABYLON.MeshBuilder.CreateTorus(
        `avatarScanLine${i}`,
        {
    ...
    const markerMaterial = mat(
      scene,
      "avatarMarkerGlow",
      new BABYLON.Color3(0.38, 0.9, 1),
      { glow: 0.7 },
    );
    const markers = [];
